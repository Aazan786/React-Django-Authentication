...
from authapi.serializers import UserRegistationSerilizer, UserLoginSerilizer, ProfileSerilizer, UserChangePasswordSerializer, SendPasswordResetEmailSerializer, PasswordResetSerializer
...
def UserRegistration(request):
    if request.method == 'POST':
        ...
        return Response({"Token": token, "Registration": "succeess"},
...
def UserLogin(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return Response({'erros':{'non_field_errors': ['Email or Password is incorrect']}}, status= status.HTTP_404_NOT_FOUND)
